...
Person schema lives in `_layouts/default.html` (inline `<script type="application/ld+json">`). Current properties:
- Identity: `name`, `alternateName`, `gender`, `description`, `disambiguatingDescription`, `image`
...
- Works: `author` (7 `ScholarlyArticle` entries from `_data/publications.yml`)

When adding new publications, update both `_data/publications.yml` **and** the `author` array in `_layouts/default.html`.
