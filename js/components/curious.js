(function(){
  ...
  var ladderCopy=[
    ...
    ['Give the claim a baseline.','“Sales are bad” tells you little without the plan, last month, competitors, or normal seasonality.'],
    ['Do not marry the first story.','A late reply could mean overload, uncertainty, forgetfulness, or low interest. Keep the alternatives open until the evidence separates them.'],
    ['Put the belief at risk.','If price causes churn, a discount should keep some customers. If it does not, look at onboarding, value, or fit.'],
    ['Look for what is missing.','Who left before complaining? Which data was never collected? Who stays quiet because dissent costs them?'],
    ['Learn by doing.','Try a small pricing test, a short project, or a changed condition. A reversible action often teaches more than another debate.']
  ];
  ...
  var hypothesis=$('#cq-hypothesis');$('#cq-build-test').addEventListener('click',function(){var h=hypothesis.value.trim();if(!h){hypothesis.focus();return}$('#cq-expect').textContent='Write one result you should be able to observe if this claim is right: “'+h+'”';$('#cq-falsify').textContent='Name a result this explanation would struggle to explain.';$('#cq-cheap').textContent='Change one condition, watch one behaviour, or look for one piece of evidence against it.'});
  ...
  var domains={
    self:['SELF','Break identity claims into parts you can practise.','“I am terrible at networking.”','“Is the hard part starting a conversation, entering a group, remembering names, or following up?”',['What happens right before the pattern?','What am I trying to get or avoid?','Which conditions make this easier?']],
    people:['PEOPLE','Replace the label with conditions.','“She is difficult.”','“When does working together get difficult, and what is she trying to protect?”',['What seems important to them?','How do they act under pressure?','What would they call fair?']],
    work:['WORK','Find the decision behind the task.','“We need to finish this report.”','“Who uses it, what decision does it support, and what happens if we stop?”',['Where does the value come from?','What is slowing it down?','What breaks at ten times the volume?']],
